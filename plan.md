...
The caller-token export primitive now accepts only a retained read-write `AuthorizedMountRoot` and bounded relative destination, rejecting read-only capabilities before the path worker queues work. Under impersonation it opens or creates every parent with `NtCreateFile` relative to the preceding pinned handle, creates a random sibling, bounds and revalidates the same protected source handle, flushes the sibling, and commits that exact handle with `FileRenameInformationEx` relative to the pinned final parent. Prefix, traversal, separator, NUL, and ADS components fail closed; non-overwrite collision preserves the existing destination, and failure cleanup targets only the exact temporary handle. This is still writeback-commit groundwork: protected-source capability/ledger binding and the ordered periodic/final reconciliation, conflict-publication, and retained-stage lifecycle are not wired, so mounts remain unavailable.
