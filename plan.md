...
The existing caller-token export primitive now copies at most the authorized protected-source length plus one byte, verifies the same open source handle's length and last-write time before publication, flushes a random create-new sibling, limits best-effort failure cleanup to that sibling, and atomically replaces an existing destination with write-through semantics. It no longer removes the destination before rename. This is only writeback-commit groundwork: the destination is not yet derived and traversed handle-relatively from an `AuthorizedMountRoot`, and the periodic/final reconciliation lifecycle is not wired.
