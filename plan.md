...
The Windows QEMU backend constructs q35/WHPX arguments, Westmere CPU, memory/vCPU/disks, no display/monitor, virtio serial, and user networking (`crates/lsb-platform/src/windows_x86_64/qemu/argv.rs`). `QemuProcess::start` validates an absolute executable, writes per-instance logs, clears the inherited environment except `SystemRoot`/`WINDIR`, and launches with `CREATE_SUSPENDED | CREATE_NO_WINDOW`. It enumerates and opens the sole initial thread, assigns the child to either the injected service-owned Job or an SDK-only internal kill-on-close Job, and resumes only after containment; the injected path never creates a second Job, and every acquisition, assignment, and resume mismatch fails closed and reaps the child (`qemu/process.rs`). Production `ManagedVm` retains its 8-process, guest-memory-plus-2-GiB Job outside the VM worker and force-terminates it at the stop deadline; a worker that remains stuck after the bounded post-termination grace aborts the service rather than being detached behind SCM `STOPPED`. Before any assignment, the Job is associated with a private completion port. The VM control loop drains at most 64 typed notifications per tick, tracks every QEMU/helper PID, and terminates containment on limit events, malformed keys/PIDs/messages, duplicate admissions, unknown exits, excessive batches, port failure, or active-process-zero during runtime. The service reserves the sandbox ledger without replacement, persists QEMU intent immediately before suspended creation, commits the PID/creation-time proof after authoritative-Job assignment and before resume, and removes the transaction only after successful VM and protected-instance teardown. On restart, a retained committed record opens one process handle and requires exact creation-time and verified-bundle image equality before termination; absence converges idempotently, mismatch quarantines, and transient query/termination/wait failures retain health-only state. Intent-only records select no PID, and non-QEMU records remain retry-required. Remaining CON-01/SEC-02 work is real completion-port/helper-tree and nested enterprise-Job proof, every non-QEMU proof adapter, and real SCM/LocalSystem crash/reboot evidence. Boot validates assets, starts QEMU, opens the guest named pipe, and waits up to 90 seconds for readiness, terminating on failure (`qemu/boot.rs`). QEMU discovery still considers environment, config, managed tools, and `PATH` outside service mode (`qemu/discovery.rs`); service mode uses only the verified managed absolute path.
...
| File transfer | — | — | — | 64 MiB/RPC, 20 GiB staging/session, 4 GiB/file |
...
The safe staged-sync backend is an implementation requirement, not an insecure compatibility switch. It creates a protected per-mount staging directory, copies the authorized snapshot into it while impersonating the client, and shares only staging. `direct-ro` continuously applies authorized host changes to staging and makes the SMB share read-only. `direct-rw` additionally applies guest staging changes back through handle-relative operations while impersonating the client, so created output receives user ownership/ACLs and no temporary-account SID reaches the caller tree. Watchers are hints; reconciliation runs at least once per second while dirty and periodically while apparently idle to catch loss/overflow. Maintain a per-relative-path baseline `{file ID, size, last-write time, content hash when needed}`; if host and guest both change an entry, return `MOUNT_CONFLICT`, stop propagation/sandbox safely, and preserve the protected staging copy for an explicit user-context export rather than overwrite either side. Existing reparse/link/entry/byte rules apply on every propagation. Guest SMB completion is not represented as a durable host-tree commit when this backend is selected; normal stop performs a final flush, while the protected 20 GiB/session and 4 GiB/file staging quotas are the hard crash-loss bounds. `HealthCheck` reports available direct backends and `StartSandbox` reports the backend chosen for every mount, so SeaWork can disclose staged durability rather than assume pinned semantics.

The caller-token export primitive now accepts only a retained read-write `AuthorizedMountRoot` and bounded relative destination, rejecting read-only capabilities before the path worker queues work. After initial staging, `StagedMount` retains a no-delete-sharing, no-follow `ProtectedStagingRoot` whose exact directory identity is committed to the protected ledger. Export accepts that capability plus a bounded relative source, opens every source component handle-relatively before impersonation, and requires a regular non-reparse file. Under impersonation it opens or creates every destination parent relative to the caller-root pin, creates a random sibling, bounds and revalidates the same source handle, flushes the sibling, and commits that exact handle with `FileRenameInformationEx` relative to the pinned final parent. Prefix, traversal, separator, NUL, and ADS components fail closed on both sides; non-overwrite collision preserves the existing destination, and failure cleanup targets only the exact temporary handle. Neither export side now accepts a naked path.

Each prepared staged mount now owns a fail-closed reconciliation controller around its baseline. Dirty hints become due within one second, idle full scans every 30 seconds, and stop's final flush share one fixed 30-second deadline. Planning preflights all conflicts before exposing an operation, orders deepest-first deletions before parent-first directory/file creation, advances the baseline only after explicit whole-cycle success, rejects foreign/stale plans with a random controller identity plus monotonic epoch, retains notifications arriving during an in-flight cycle, and requires a catch-up final cycle rather than completing with newer dirt. Any reported I/O failure makes the controller terminal. The path worker can freshly snapshot both the authorized host root under the retained caller token and the protected stage under the service token. Enumeration is performed on each exact directory handle, every name is validated and reopened relative to its parent pin, unsafe attributes/multi-link/repeated identities fail closed, and file hashing plus final metadata validation use the same open handle. Initial staging derives its baseline from the pinned protected root. The controller is private to `StagedMount`; when a cycle is due, the mount requires the same recorded host identity, caller SID, and access mode before obtaining both fresh snapshots. A capability mismatch or observation failure makes the controller terminal before it exposes a plan. Host-import and guest-export primitives verify the observed source through its retained capability, then apply ordered file, directory, type-change, or exact-handle deletion mutations to the opposite pin; file publication is content-hash checked, flushed, and atomically renamed from a random sibling. `StagedMount::execute_plan` dispatches the whole plan, freshly snapshots both sides, and advances the baseline only when both semantically equal the planned result; any operation or proof failure is terminal. Divergent paths are retained in the terminal controller and queryable from the mount alongside the pinned protected stage, and the mount exposes bounded change/final-flush control methods without exposing the controller. Conflict artifact publication/recovery journaling, the Windows host monitor and guest-event/periodic/final runtime driver, retained-stage user export, VM stop/cleanup integration, and SMB lifecycle remain unwired, so the service still advertises no mount capability.
