...
results_path="${LSB_OFFICE_SMOKE_RESULTS:-target/office-rootfs-smoke.jsonl}"
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
repo_root="$(cd "$script_dir/.." && pwd)"

usage() {
    cat <<'EOF'
Usage: scripts/validate-office-rootfs.sh [--iterations N] [--lsb PATH] [--results PATH]

Runs an ephemeral VM office workload and writes one JSON record per iteration.
Set LSB_KERNEL, LSB_INITRD, and LSB_ROOTFS to test unpublished runtime assets.
EOF
}
...
results_parent="$(dirname "$results_path")"
...
for ((iteration = 1; iteration <= iterations; iteration += 1)); do
    ...
    guest_result="$(
        "$lsb_binary" run \
            ...
            -- /usr/bin/env LSB_SMOKE_NETWORK=1 node /workspace/office-rootfs-smoke.mjs
    )"
