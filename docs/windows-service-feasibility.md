...
- The client now pins the configured service process, single-link non-reparse executable, and each non-reparse package directory from `ProgramFiles\SeaWork` through `bin`, requires no-UI Authenticode trust and the release-compiled SHA-256 publisher thumbprint, then repeats the full SCM identity/configuration query before sending Hello. Every held package object must be owned by SYSTEM, Administrators, or TrustedInstaller and have no applicable write/delete/ACL/owner-control allow ACE for any other principal; null, malformed, unreadable, unfamiliar compound-write, or untrusted descriptors fail closed. Windows release clients fail to build without publisher policy, and the Node release workflow consumes the same variable as the service release. Local compilation, all 21 client tests, and strict Clippy pass. This is source-level SEC-01 evidence only: installed-tree ACL inspection, publisher-rotation overlap, and real signed-artifact adversarial SCM/squatter/replacement tests remain pending.
...
| Client pre-Hello service/package pinning | Source tests passed | Current source; 2026-07-20 | Process/image identity, release-compiled Authenticode publisher, known-folder package chain, protected owner/DACL policy, non-reparse/final-path pins, and second SCM query implemented; installed ACL/rotation proof plus signed-artifact adversarial runtime evidence remain required |
