...
- NET-02 quota accounting now reserves mappings atomically at the fixed 32/sandbox, 64/user, and 128/global ceilings and bounds active tunnels at 128/sandbox. Deterministic tests fill every boundary, prove failed reservations do not consume capacity, release and reuse exact slots, reject duplicate sandbox reservations, require a nonempty mapping reservation before a tunnel, and clear all tunnel/mapping state on sandbox release. The full serial service run passes 125 tests with one helper ignored. This quota book is source-only and is not connected to listeners while the relay/WFP capabilities remain disabled.
- NET-02 now has a bounded fail-closed service contract: the Node service API preserves exact nonzero host/guest port mappings, `network.exposeHost` defaults only its guest port to the explicitly requested host port, duplicate exposed guest ports and more than 32 combined mappings are rejected, and an owner-token-relay feature bit protects the new wire field from older services. The client understands that bit, but the service deliberately does not advertise it; host ports still return `PORT_ISOLATION_UNAVAILABLE` and expose-host requests fail negotiation before admission. Protocol, client, Node Rust, declaration, TypeScript, and source tests pass. No listener, WFP filter, or LocalSystem loopback connection was enabled, and the relay implementation plus real WFP/multi-user evidence remain pending.
...
- The authenticated reader's exact 64-frame dispatch queue now has deterministic saturation and fault tests. In-memory and uniquely named real Windows kernel-pipe runs fill it plus two upstream frames, proving backpressure recovery, ordered lossless delivery, writer unblocking, and restored capacity; the real pipe uses a two-header kernel buffer and exists only for the standard-token test lifetime. An invalid-magic frame is reported once, stops the reader, and prevents a following valid frame from being dispatched. Four authenticated in-memory connections also hold their full 16-request quotas to exhaust the shared 64-request semaphore: a fifth connection receives `QUOTA_EXCEEDED`, then occupies exactly one slot released by cancellation. The full serial service run passes 122 tests with one child helper intentionally ignored. This is source and standard-token IPC evidence, not installed service endpoint multi-client load/resource behavior or broader transport fault injection.
- Mount authorization now retains protected-root handles and compares full `FILE_ID_INFO` identities against the complete pinned mount chain. System/service roots are never exempted; ProfileList aliases of the authenticated caller are excluded only from the profile-derived deny identities. Pure policy tests and a current-token relative/direct-handle identity regression pass, and the full serial service run passes 121 tests with one helper intentionally ignored. This is source-level MNT-01 evidence; real NTFS/ReFS alternate-name, SUBST/short-name, GPO-relocated-profile, and concurrent path-swap fixtures remain pending, and mounts remain disabled.
- Staged mounts now start a recursive host monitor from the retained authorized root before initial copying. The monitor reopens that exact identity handle-relatively, bounds/coalesces notifications, converts overflow to a full-rescan hint, invalidates an in-flight plan on failure, and waits for outstanding overlapped I/O cancellation before teardown; `StagedMount::plan_due` drains the hints into the one-second dirty controller while 30-second authoritative snapshots remain the loss detector. Exact standard-token nested-change/cancellation and controller regressions plus the 143-test serial non-Job service run pass. This is source/test-binary MNT-01 evidence only: no SMB share, account/right/ACE, SCM/LocalSystem, NTFS/ReFS overflow/timing, guest-event, or adversarial rename test was run, and mounts remain disabled.
...
| Production/development endpoint isolation | Source/build tests passed | Current source/test binaries; 2026-07-20 | Non-default feature selects distinct SCM, pipe, and ProgramData identities across protocol/service/client/Node; the deterministic packager emits profile-specific contracts/artifact names and rejects profile-mismatched PEs during staging and archival without relaxing publisher/catalog/ACL policy; production release service PE contains production and no development identity strings; elevated development installation and any explicit unsigned-client policy remain pending |
| Authenticated dispatch saturation/fault | Source and standard-token test passed | Current source/test binary; 2026-07-20 | Exact 64-frame reader capacity, real kernel-pipe two-header buffering/two-frame upstream pressure, ordered recovery, malformed-frame termination, and 4x16 global request exhaustion/rejection/single-slot reuse proven; installed endpoint multi-client load remains required |
| Canonical protected-root mount identity | Source and standard-token test passed | Current source/test binary; 2026-07-20 | Retained no-delete-share root pins and 64-bit-volume/128-bit-file identities cover the entire mount chain with caller-profile-only exception; real alias/path-swap fixtures remain required and mounts stay unavailable |
| Pinned staged host monitor | Source and standard-token test passed | Current source/test binary; 2026-07-21 | Exact handle-relative root reopen, recursive bounded/coalesced hints, overflow-to-rescan behavior, and cancellation completion are implemented; LocalSystem/SMB/NTFS/ReFS overflow/timing, guest-event, and adversarial rename evidence remain required and mounts stay unavailable |
