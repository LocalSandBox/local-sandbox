...
- Summary: Added a private Windows QEMU direct boot orchestration module and wired `PlatformVm::start()` to run QEMU discovery/preflight, build direct Linux boot argv through the existing builder, launch through the existing supervisor/Job Object path, capture stdout/stderr/serial/preflight/status artifacts, and require non-empty Linux serial output before recording the M05-only boot result. The provisioned smoke path now uses `-cpu Westmere` for WHPX compatibility after `-cpu max` failed on the self-hosted runner, and the x86_64 kernel defconfig enables the 8250 serial console used by `console=ttyS0`. Later control, readiness, exec, mounts, networking, and checkpoints remain unsupported with explicit errors.
- Tests run: See `../state.md` for the final validation log. Local validation passed `cargo fmt --all -- --check`, targeted QEMU argv/boot tests, `cargo check --workspace`, `cargo test --workspace`, `cargo check -p lsb-platform --target x86_64-pc-windows-msvc`, and `git diff --check`. Full macOS-hosted workspace Windows target check remains blocked by external MSVC C/assembler tooling. Windows self-hosted `./scripts/win-gh-test smoke` passed in run `28698120131`; it ran real QEMU/WHPX preflight and `windows_qemu_boot_smoke`, observing QEMU alive for 10000 ms with provisioned kernel/initrd/rootfs assets and non-empty serial output.
- Debug artifacts: M05 writes `qemu.argv.redacted.txt`, `qemu.stdout.log`, `qemu.stderr.log`, `qemu.status.json`, `serial.log`, `preflight.json`, and `boot.status.json` under `<instance-dir>/diagnostics`, or under `LSB_WINDOWS_BOOT_ARTIFACT_DIR` for the ignored smoke test. The successful hardware smoke wrote `C:\lsb-assets\work\28698120131-1\diagnostics` and uploaded staged diagnostics in `windows-lsb-diagnostics` artifact ID `8079375534`; `boot.status.json` recorded `serial_observed_alive`, and `serial.log` was 15762 bytes with kernel, `/init`, rootfs mount, and `lsb-guest` startup lines.
