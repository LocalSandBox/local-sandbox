...
| R006 | CAS/NBD store depends on Unix domain sockets | Medium | High | M13 implements simple Windows checkpoint artifacts first with qcow2 overlays and flattened qcow2 checkpoints; port CAS/NBD or another deduplicated store later if needed. | Mitigating | TBD |
