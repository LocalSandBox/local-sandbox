...
- Last verified implementation commit: `1a50922` (`feat(client): expose maintenance operations`)
...
- Phase 5 RPC verification: 49 service, 6 Rust client, and 19 protocol tests pass; affected service/client/protocol Clippy passes with `--no-deps -D warnings`
...
- Phase 5 backlog: propagate request cancellation cooperatively into blocking VM boot, guest command, file, and cleanup work; the live dispatcher currently stops the RPC future and returns `CANCELLED`, but an already-running blocking worker can finish independently. Raise `ReadFile`/`WriteFile` beyond 256 KiB only when file credit replenishment is live. Maintenance publisher verification has compile/unit coverage but still needs an installed signed maintenance executable and protected production config. Real start/exec/spawn/file/watch/stop/update/uninstall remains unverified because this shell has no installed bundle assets or Session 0/WHPX fixture; no hardware-dependent test was forced.
...
5. Finish Phase 5 cooperative worker cancellation and live file credit using only owner-bound service resources.
