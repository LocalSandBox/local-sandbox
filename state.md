...
- Last verified implementation commit: `9e492ea` (`feat(service): stream bounded guest file reads`)
...
- Phase 5 RPC verification: 35 service, 2 Rust client, and 15 protocol tests pass; affected service/client/protocol Clippy passes with `--no-deps -D warnings`
...
- Phase 5 stream foundation: protocol types now define bounded cancel/window-update/event/close controls and sequenced binary stream payloads. `ReadFile` uses a random stream ID, strict declared length and per-stream sequence checks, and binary `StreamData` frames end to end through Rust and Node. Until window-update dispatch is connected, reads are capped at the initial 256 KiB credit and larger files fail explicitly instead of buffering without bound.
- Phase 5 backlog: add owner-bound spawn/kill, binary `WriteFile`, watch, live credited window-update/event/cancel dispatch, admin update/uninstall RPCs, full bounded concurrent dispatcher/writer integration, and extend the Node remote objects with those completed operations. Raise `ReadFile` beyond 256 KiB only when credit replenishment is live. Real start/exec/file/stop remains unverified because this shell has no installed bundle assets or Session 0/WHPX fixture; no hardware-dependent test was forced.
