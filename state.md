...
Phases 0-6 upstream shipping foundations are implemented; production signing, downstream installer integration, and the real-machine acceptance matrix remain explicitly gated.
...
- Last verified implementation commit: `62f4f44` (`feat(release): gate service runtime dependencies`)
...
- Phase 2 backlog: real two-user/two-logon SCM tests, general-client Authenticode publisher enforcement, service-SID/config2 verification, active process-exit monitoring, handshake/rate limits wired into the accept loop, and queue/backpressure fault injection. The client verifies SCM PID/type/account/path before sending bytes, and the service derives/cross-checks OS token identity before Hello. Maintenance clients are separately restricted to elevated Administrator tokens and Authenticode-valid images below protected configured roots whose signer SHA-1/SHA-256 thumbprint is allowlisted.
...
- Phase 5 stream foundation: protocol types define bounded cancel/window-update/event/close controls and sequenced binary stream payloads. `ReadFile` and `WriteFile` use random stream IDs, strict declared lengths and per-stream sequence checks, and binary `StreamData` frames end to end through Rust and Node. File transfers are capped at 64 MiB and now use live 256 KiB initial credit in both directions: the service consumes client upload chunks before granting more credit, and the client consumes download chunks before replenishing the service. Server WindowUpdate and Event frames share one strict sequence, stream registrations are removed on every response/write failure path, and nonterminal empty chunks are rejected. Spawned stdout/stderr use validated replenishment up to a 4 MiB window, a single writer capped at 128 frames/16 MiB, strict control sequencing, and bounded client channels. Late credit for a closed stream is harmless and connection drop releases stream state. The pipe reader dispatches up to 16 requests per connection and 64 globally, permits out-of-order responses, handles sequenced Cancel/WindowUpdate controls while operations run, applies server deadlines, and cancels every active RPC on close/break.
...
- Phase 5 cancellation foundation: request cancellation and deadlines propagate through dispatcher/RPC/session/VM command contexts; queue waits poll cancellation, unary exec can kill its bounded managed process, boot rootfs copy checks between chunks and cleans a cancelled startup, and new spawn/watch resources are finalized or retired on cancellation. `WriteFile` checks before the atomic rename and removes its temporary sibling when cancelled. The remaining narrow limitation is that an opaque synchronous SDK filesystem call already in progress cannot be interrupted mid-call, so that individual mutation may finish before the post-call cancellation check; no RPC result or resource ownership escapes.
...
- Phase 6 foundation: xtask now stages only a fresh, bounded, reparse-free closed `LocalSandbox/` tree, emits exact service/bundle contracts, verifies AMD64 PE and pinned runtime/QEMU metadata, and creates deterministic payload/symbol ZIPs plus `SHA256SUMS`. The service performs an adjacent installed-layout manifest/hash/contract check. Release scripts generate a full locked SPDX/license inventory, reject non-system runtime DLLs and dynamic VC runtime imports, sign/timestamp the PE and full catalog with exact subject/SHA-256 pinning, and verify catalog membership. The conditional release workflow preserves the PDB, uses static CRT, attests both ZIPs, and publishes no unsigned service when signing is disabled. The operational contract, installer recipes, publisher rotation, release checklist, and hard external gates are in `docs/seawork-service-release.md`.
- Phase 6 external gates: the development PFX was tested only with `AllowUntrustedTestCertificate` and `SkipTimestamp`; shipping requires an organization-controlled production identity, trusted chain, RFC 3161 timestamp, and clean-machine verification. SeaWork must implement malicious-ZIP rejection, protected extraction, digest/publisher pinning, exact SCM/Event configuration, maintenance transactions, final-path reverification, and helper removal. Elevated Session 0/WHPX/SMB/two-user/crash-reboot/update-rollback-uninstall and managed-fleet EDR/GPO/proxy/VPN evidence remains release-blocking.
