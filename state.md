...
- Last verified implementation commit: `3f89d75` (`feat(node): stream managed service processes`)
...
- Phase 5 RPC verification: 37 service, 4 Rust client, and 17 protocol tests pass; affected service/client/protocol Clippy passes with `--no-deps -D warnings`
- Phase 5 Node verification: the Windows binding compiles; N-API generation succeeds; 8 focused declaration/package tests pass; the built addon exports `SeaWorkService`, `SeaWorkSandbox`, and `SeaWorkProcess`
...
- Phase 5 Node foundation: the existing N-API package now exports experimental `SeaWorkService`/`SeaWorkSandbox` remote objects backed only by `lsb-service-client`. Its generated `SeaWorkStartOptions` contains only CPU, memory, and disk, and the API-shape test rejects trusted path/identity fields. Health, lifecycle, bounded exec, guest filesystem methods, and credited spawned processes are exposed without JavaScript pipe/security code. `SeaWorkProcess` provides independently consumable bounded stdout/stderr chunks, async kill, and ordered exit; the existing direct `Sandbox` API remains for compatibility until Phase 6 cutover.
...
- Phase 5 backlog: add watch/stop-watch, cancel dispatch, admin update/uninstall RPCs, full bounded concurrent request dispatch, and extend the Node remote objects with those completed operations. Raise `ReadFile`/`WriteFile` beyond 256 KiB only when file credit replenishment is live; `WriteFile` still commits directly rather than through the plan's temporary guest-file rename. Real start/exec/spawn/file/stop remains unverified because this shell has no installed bundle assets or Session 0/WHPX fixture; no hardware-dependent test was forced.
